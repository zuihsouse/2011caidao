...
官方网站：http://www.zhongguocaidao.com/
----------------------------------------------------------------------------------------------------------
免责申明：
...
　　命令解释:
	A) 查单一IP的绑定域名
	{reverse_ip} {url:http://www.zhongguocaidao.com/}
	B) 扫描本C段开放的WEB服务器，并查询绑定域名
	{reverse_ip_c} {url:http://www.zhongguocaidao.com/}
	C) 只扫描本C段开放的WEB服务器
	{reverse_ip_c} {url:http://www.zhongguocaidao.com/} {port}
	D) 蜘蛛爬行
	{spider} {url:http://www.zhongguocaidao.com/}
	E) 蜘蛛爬行，并设定爬行范围
	{spider} {url:http://www.zhongguocaidao.com/} {range:zhongguocaidao.com}
	F) 蜘蛛爬行，过滤重复URL加快速度
	加上 {filter}
	G) 爆破功能，%s为dict中的一行
	...
	{crack} {url:http://www.zhongguocaidao.com/%s/} {flag:successfully} {dict:list.txt}
